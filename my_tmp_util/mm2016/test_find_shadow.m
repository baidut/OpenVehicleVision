...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function plot_rgb()
        ...
        %% plot rgb
        rowRange = floor(Raw.rows*5/8):floor(Raw.rows*6/8);
        col = ceil(Raw.cols*3/4); %ceil(Raw.cols/2);
        figure,maxfig
        subplot(3,2,[1 5]); imshow(Raw);hold on;
        x = ones([numel(rowRange) 1])*col;
        y = rowRange;
...
%         low_half = floor(Raw.rows/2):floor(Raw.rows*3/4); %floor(Raw.rows/2):Raw.rows;
        ...
        show_rgb(col, rowRange);
        subplot(324);
        show_g_minus_b(col, rowRange);
        subplot(326);
        show_ii(col, rowRange, 'c');
        hold on;
%         show_ii(ceil(Raw.cols/2), rowRange, 'y');
        ...
        function show_rgb(c, rows) % rows c
...
%             plot(X, (1-SF)*ratio, 'b', 'LineWidth', 1);
        ...
        function show_g_minus_b(c, rows) % rows c
            % only show the low half image
            ratio = 20; % scale the y axis
            ...
            X = rows - rows(1);
            plot(X, (Gd(rows,c)-Bd(rows,c)+0.5)*ratio, 'c', 'LineWidth', 3);
            plot(X, (ones([numel(rows),1]))*ratio*0.5, 'w');
%             SF = Gd(rows,c) - 2*Bd(rows,c);
%             plot(X, (1-SF)*ratio, 'b', 'LineWidth', 1);
            %             axis on;
            % explain x and y
        end
        function show_ii(c, rows, varargin) % rows c
            % only show the low half image
            ratio = 20; % scale the y axis
            ...
            X = rows - rows(1);
            C = ones([numel(rows),1]); % constant line
            II = (Gd(rows,c)+0.22-Bd(rows,c))./Bd(rows,c); %  0.22 0.50645
            maxII = max(II(:));
            minII = min(II(:));
            
            r = 5; % 20
            plot(X, II*r, varargin{:}, 'LineWidth', 3);
            plot(X, C*maxII*r, 'w');
            plot(X, C*minII*r, 'w');
            plot(X, C*ratio, 'w');
            title(num2str(maxII-minII));
%             SF = Gd(rows,c) - 2*Bd(rows,c);
%             plot(X, (1-SF)*ratio, 'b', 'LineWidth', 1);
            %             axis on;
            % explain x and y
